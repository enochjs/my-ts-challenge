...
// ============= Your Code Here =============
type Arr2Union<T, R = never> = T extends [infer F, ...infer Rest] ? Arr2Union<Rest, F | R> : R   

type RequiredByKey<T, K extends string[], U = Arr2Union<K>> = Merge<{
...
type JSONSchema2TS<T> = 
  T extends { properties: infer V, required: infer K extends string[]}
    ? RequiredByKey<{ [P in keyof V]?: JSONSchema2TS<V[P]> }, K> 
    : T extends { properties: infer V }
      ? { [P in keyof V]?: JSONSchema2TS<V[P]> }
