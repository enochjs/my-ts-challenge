...
type IOptions<S, G, A> = {
  id: string;
  state: (this: never) => S
  getters: G & ThisType<Readonly<S> & IResultGetter<G>>
  actions: A & ThisType<S & Readonly<IResultGetter<G>> & IActionsResult<A>>
}

type IResult<S, G, A> = MergeType<S & IResultGetter<G> & A>

declare function defineStore<S, G, A>(store: IOptions<S, G, A>): IResult<S, G, A>
