...
type StoreOption<S, G, A> = {
  id: string
  state: () => S
  getters: G & ThisType<Readonly<S> & GetMapValue<G>>
  actions: A & ThisType<S & A>
}

type IStoreReturn<S, G, A> = S & A & GetMapValue<G>

declare function defineStore<S, G, A>(store: StoreOption<S, G, A>): IStoreReturn<S, G, A>
