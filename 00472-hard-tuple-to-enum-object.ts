...
type MergeReadonlyType<T> = {
  readonly [P in keyof T]: T[P]
}

type Capitalize<S extends string> = S extends `${infer F}${infer Rest}` ? `${Uppercase<F>}${Rest}` : S

type Enum<T extends readonly string[], N extends boolean = false, C extends number[] = [], R extends Record<string, string | number> = {}> =
  T extends readonly [infer F extends string, ...infer  Rest extends string[]]
    ? Enum<Rest, N, [...C, 1], MergeReadonlyType<R & { [K in F as Capitalize<K>]: N extends true ? C['length'] : F }>>
    : R
